...
class TaskConditionedPooling(nn.Module):
    def __init__(
        ...
        # Generic learnable pooling query.
        # This is similar to the query in HybridAttentionPooling.
        self.base_query = nn.Parameter(torch.randn(1, 1, input_dim))

        # Turns the pooled task/context representation into a query shift.
        # This is what makes the pooling conditioned on the current input.
        ...
        # Token scoring branch, same idea as HybridAttentionPooling.
        self.attn_mlp = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
            nn.Tanh(),
        ...
        # Query attention branch.
        # The query is now task-conditioned instead of fixed.
        self.mha = nn.MultiheadAttention(
            embed_dim=input_dim,
            num_heads=num_heads,
            batch_first=True,
        ...
        # Learnable interpolation between MLP scores and MHA scores.
    ...
    def forward(self, x, mask=None):
        ...
        # mask: [B, T], where True/1 means real token
        key_padding_mask = ~mask.bool() if mask is not None else None
        mask_expanded = mask.unsqueeze(-1) if mask is not None else None

        # Build a rough task/context representation.
        # Since I-FailSense currently does not pass a text-only mask,
        # this pools over all valid tokens, including image and text tokens.
        if mask_expanded is None:
            task_repr = x.mean(dim=1)
        else:
            valid = mask_expanded.to(x.dtype)
            task_repr = (x * valid).sum(dim=1) / valid.sum(dim=1).clamp(min=1)

        # Turn task/context representation into a query shift.
        query_shift = self.task_to_query(task_repr).unsqueeze(1)

        # Final task-conditioned query.
        query = self.base_query.expand(B, -1, -1) + query_shift

        # Branch 1: MLP token scores.
        mlp_scores = self.attn_mlp(x)  # [B, T, 1]

        # Branch 2: task-conditioned query attention scores.
        ...
        # Combine both scoring branches.
        combined_scores = self.alpha * mlp_scores + (1 - self.alpha) * mha_scores
        combined_scores -= combined_scores.max(dim=1, keepdim=True).values

        # Remove padding tokens from pooling.
        if mask_expanded is not None:
            combined_scores = combined_scores.masked_fill(
                ~mask_expanded.bool(),
        ...
        # Convert scores into pooling weights.
        weights = F.softmax(combined_scores, dim=1)
        weights = self.attn_dropout(weights)

        # Weighted average over token features.
        pooled = torch.sum(weights * x, dim=1)  # [B, D]
...
class FailSenseExp(nn.Module):
    def __init__(
        ...
        self.att_poolings = nn.ModuleList(
            [
                TaskConditionedPooling(feature_dim).to(self.device)
