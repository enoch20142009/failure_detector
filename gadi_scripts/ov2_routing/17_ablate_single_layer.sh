...
#PBS -j oe
#
# A7 — NGF ablation: inner guiding on BASE only (L=1, empty vision_layer_indices).
# Collapses NGF to a single guided dual-query field on V_base, i.e. the flat
# router class (~ job 12). Lower bound for the multi-depth nesting.
#
#   qsub gadi_scripts/ov2_routing/17_ablate_single_layer.sh
#
set -euo pipefail
...
RESULT_DIR=./results_ov2_calvin_ngf_a7_single
EVAL_IN=./eval_results/ov2_calvin_ngf_a7_single_indomain
EVAL_X=./eval_results/ov2_calvin_ngf_a7_single_to_droid

echo "==== A7 train: NGF base-only (L=1) guiding on CALVIN-1p ===="
