...
#PBS -j oe
#
# Routed MaTCA (dual task/failure query router) on CALVIN-1p: train in-domain,
# then evaluate in-domain (CALVIN) and cross-dataset (DROID), with the grounding
# probe + a task-substitution intervention. Routed runs use batch_size=1.
#
#   qsub gadi_scripts/ov2_routing/03_train_eval_calvin_routed.sh
#
set -euo pipefail
...
RESULT_DIR=./results_ov2_calvin_routed
EVAL_IN=./eval_results/ov2_calvin_routed_indomain
EVAL_X=./eval_results/ov2_calvin_routed_to_droid

echo "==== train routed on CALVIN-1p ===="
