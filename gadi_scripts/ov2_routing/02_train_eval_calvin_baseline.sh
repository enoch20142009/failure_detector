...
#PBS -j oe
#
# Baseline MaTCA head on CALVIN-1p: train in-domain, then evaluate in-domain
# (CALVIN) and cross-dataset (DROID). No Stage-1 routing.
#
#   qsub gadi_scripts/ov2_routing/02_train_eval_calvin_baseline.sh
#
set -euo pipefail
...
RESULT_DIR=./results_ov2_calvin_baseline
EVAL_IN=./eval_results/ov2_calvin_baseline_indomain
EVAL_X=./eval_results/ov2_calvin_baseline_to_droid

echo "==== train baseline on CALVIN-1p ===="
