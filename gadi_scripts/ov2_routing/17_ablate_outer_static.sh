...
#PBS -j oe
#
# A3 — NGF ablation: inner guiding ON; outer alpha_l is STATIC learnable
# (no task-text MLP). Isolates task-conditioned depth vs a fixed depth mix.
#
#   qsub gadi_scripts/ov2_routing/17_ablate_outer_static.sh
#
set -euo pipefail
...
RESULT_DIR=./results_ov2_calvin_ngf_a3_static
EVAL_IN=./eval_results/ov2_calvin_ngf_a3_static_indomain
EVAL_X=./eval_results/ov2_calvin_ngf_a3_static_to_droid

echo "==== A3 train: NGF inner guiding + STATIC alpha on CALVIN-1p ===="
python src/finetune_FS_ov2_routed.py \
  ...
  --use_nested_guided_fusion --ngf_layer_weight_mode static \
