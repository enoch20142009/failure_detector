...
#PBS -j oe
#
# Fuse-then-route + merger adapter on CALVIN-1p.
# Stage 1a: TGIF-style multi-depth FEATURE fusion (static weights on layers 9/17/24).
# Stage 1b: one flat dual-query router on fused tokens (not per-depth delta fusion).
# Stage 1.5: trainable parallel adapter on frozen patch merger.
# Stage 2: task-agnostic hybrid MaTCA pooling.
#
# Compare against:
#   03  flat router only
#   09  hier router (per-depth delta fusion — collapsed)
#   12  flat router + merger adapter
#
#   qsub gadi_scripts/ov2_routing/14_train_eval_calvin_fuse_then_route_merger_adapter.sh
#
set -euo pipefail
...
RESULT_DIR=./results_ov2_calvin_fuse_then_route_merger_adapter
EVAL_IN=./eval_results/ov2_calvin_fuse_then_route_merger_adapter_indomain
EVAL_X=./eval_results/ov2_calvin_fuse_then_route_merger_adapter_to_droid

echo "==== train fuse-then-route + merger adapter on CALVIN-1p ===="
