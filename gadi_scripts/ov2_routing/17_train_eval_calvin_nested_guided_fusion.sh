...
# NGF-0 (primary): Nested Guided Fusion on CALVIN-1p.
#   Inner : per-(layer,patch) dual-query guiding gates on layers 9/17/24 + base.
#   Outer : TGIF-style task-text layer weights alpha_l (softmax over 4 depths).
#   Fuse  : F_n = sum_l alpha_l * h_{l,n}; F fully replaces V_base.
#   Post  : trainable merger adapter -> frozen merger/LLM -> tcond MaTCA head.
#
...
RESULT_DIR=./results_ov2_calvin_ngf_v2
EVAL_IN=./eval_results/ov2_calvin_ngf_v2_indomain
EVAL_X=./eval_results/ov2_calvin_ngf_v2_to_droid

echo "==== train NGF-0 (nested guided fusion) on CALVIN-1p ===="
