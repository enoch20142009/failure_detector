...
#PBS -j oe
#
# M6 task-only outer — inner off, TextLayerRouter (no contrastive outer).
#   qsub gadi_scripts/ov2_routing/33_train_eval_m6_task_only_outer.sh
#
set -euo pipefail
module purge
...
RESULT_DIR=./results_ov2_calvin_m6_task_only_outer
EVAL_IN=./eval_results/ov2_calvin_m6_task_only_outer_indomain
EVAL_X=./eval_results/ov2_calvin_m6_task_only_outer_to_droid
...
echo "Done M6 task-only outer: ${RESULT_DIR}"
