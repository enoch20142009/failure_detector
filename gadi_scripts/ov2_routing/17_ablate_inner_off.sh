...
#PBS -j oe
#
# A2 — NGF ablation: inner guiding OFF (h_l = raw V_l); text alpha_l only.
# TGIF-style outer fusion with no per-patch dual-query grounding.
#
#   qsub gadi_scripts/ov2_routing/17_ablate_inner_off.sh
#
set -euo pipefail
...
RESULT_DIR=./results_ov2_calvin_ngf_a2_inner_off
EVAL_IN=./eval_results/ov2_calvin_ngf_a2_inner_off_indomain
EVAL_X=./eval_results/ov2_calvin_ngf_a2_inner_off_to_droid

echo "==== A2 train: NGF inner OFF + text alpha on CALVIN-1p ===="
