...
#PBS -j oe
#
# Same as job 14 but with QMSA-style additive-bias guiding gates instead of
# multiplicative sigmoid gates on the dual-query router.
#
#   qsub gadi_scripts/ov2_routing/15_train_eval_calvin_fuse_then_route_merger_adapter_guiding.sh
#
set -euo pipefail
...
RESULT_DIR=./results_ov2_calvin_fuse_then_route_merger_adapter_guiding
EVAL_IN=./eval_results/ov2_calvin_fuse_then_route_merger_adapter_guiding_indomain
EVAL_X=./eval_results/ov2_calvin_fuse_then_route_merger_adapter_guiding_to_droid

echo "==== train fuse-then-route + merger adapter + guiding gates on CALVIN-1p ===="
