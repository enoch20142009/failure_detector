...
#PBS -j oe
#
# Hierarchical per-depth dual-query router on CALVIN-1p: train in-domain,
# then evaluate in-domain (CALVIN) and cross-dataset (DROID), with the grounding
# probe. Recommended pairing: --use_hier_router --pooling_mode hybrid.
#
#   qsub gadi_scripts/ov2_routing/09_train_eval_calvin_hier_routed.sh
#
set -euo pipefail
...
RESULT_DIR=./results_ov2_calvin_hier_routed
EVAL_IN=./eval_results/ov2_calvin_hier_routed_indomain
EVAL_X=./eval_results/ov2_calvin_hier_routed_to_droid

echo "==== train hierarchical routed on CALVIN-1p ===="
python src/finetune_FS_ov2_routed.py \
  ...
  --use_hier_router --router_mode contrastive --gate_type sigmoid \
  --gate_style multiplicative --depth_fusion_mode query_cond \
