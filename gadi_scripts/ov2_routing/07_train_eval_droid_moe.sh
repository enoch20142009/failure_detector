...
#PBS -j oe
#
# Mixture-of-Failure-Experts (MoE) router on DROID-1p: train in-domain, then
# evaluate in-domain (DROID) and cross-dataset (CALVIN). Reports per-expert
# usage and gate entropy. Latent experts (no failure-mode labels).
#
#   qsub gadi_scripts/ov2_routing/07_train_eval_droid_moe.sh
#
set -euo pipefail
...
RESULT_DIR=./results_ov2_droid_moe
EVAL_IN=./eval_results/ov2_droid_moe_indomain
EVAL_X=./eval_results/ov2_droid_moe_to_calvin

echo "==== train MoE on DROID-1p (4 experts, top-2, relaxed LB) ===="
