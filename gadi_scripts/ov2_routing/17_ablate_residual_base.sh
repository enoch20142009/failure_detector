...
# A5 — NGF ablation: full NGF but residual blend F = V_base + eta*(F_fused - V_base)
# with eta init 0, instead of fully replacing V_base. Replace-vs-residual test.
#
...
RESULT_DIR=./results_ov2_calvin_ngf_a5_residual_v2
EVAL_IN=./eval_results/ov2_calvin_ngf_a5_residual_v2_indomain
EVAL_X=./eval_results/ov2_calvin_ngf_a5_residual_v2_to_droid

echo "==== A5 train: NGF full, residual base blend on CALVIN-1p ===="
python src/finetune_FS_ov2_routed.py \
  ...
  --use_nested_guided_fusion --ngf_layer_weight_mode text --nested_residual \
