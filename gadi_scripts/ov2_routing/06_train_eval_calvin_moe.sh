...
#PBS -j oe
#
# Mixture-of-Failure-Experts (MoE) router on CALVIN-1p: train in-domain, then
# evaluate in-domain (CALVIN) and cross-dataset (DROID). Reports per-expert
# usage and gate entropy. CALVIN/DROID have NO failure-mode labels, so experts
# specialize latently (relaxed load-balance, no gate supervision).
#
#   qsub gadi_scripts/ov2_routing/06_train_eval_calvin_moe.sh
#
set -euo pipefail
...
RESULT_DIR=./results_ov2_calvin_moe
EVAL_IN=./eval_results/ov2_calvin_moe_indomain
EVAL_X=./eval_results/ov2_calvin_moe_to_droid

echo "==== train MoE on CALVIN-1p (4 experts, top-2, relaxed LB) ===="
