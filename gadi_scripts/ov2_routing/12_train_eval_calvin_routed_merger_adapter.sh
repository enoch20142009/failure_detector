...
#PBS -j oe
#
# Flat dual-query router + trainable merger adapter on CALVIN-1p.
# Stage 1: grounded patch routing on V_base (same as job 03).
# Stage 1.5: parallel low-rank adapter on frozen patch merger (vision→LLM).
#
#   qsub gadi_scripts/ov2_routing/12_train_eval_calvin_routed_merger_adapter.sh
#
set -euo pipefail
...
RESULT_DIR=./results_ov2_calvin_routed_merger_adapter
EVAL_IN=./eval_results/ov2_calvin_routed_merger_adapter_indomain
EVAL_X=./eval_results/ov2_calvin_routed_merger_adapter_to_droid

echo "==== train routed + merger adapter on CALVIN-1p ===="
