...
#PBS -j oe
#
# Phase 1: honest intermediate-layer transfer test.
#   * Fuse intermediate layers {6,12,18} ONLY (no V_base anchor).
#   * Tap FFN-Act (post-GELU) at those blocks, per-layer down-proj to vision_dim.
#   * TextLayerRouter alpha (ngf_layer_weight_mode=text), replace_base=True.
#   * Full trainable connector replaces the frozen merger (warm-started clone).
#   * MaTCA head unchanged. Compare CALVIN->DROID AUROC vs the 0.832 champion.
#
#   qsub gadi_scripts/ov2_routing/25_ngf_ffn_act_intermediate.sh
#
set -euo pipefail
...
RESULT_DIR=./results_ov2_calvin_ngf_ffn_act_intermediate
EVAL_IN=./eval_results/ov2_calvin_ngf_ffn_act_intermediate_indomain
EVAL_X=./eval_results/ov2_calvin_ngf_ffn_act_intermediate_to_droid

echo "==== train NGF FFN-Act intermediate-only + full connector on CALVIN-1p ===="
