...
# Arch C (novel) — sequential depth-recurrent refinement (SequentialNestedFusion):
# a running state starts at V_base and is progressively refined by each vision
# depth, with each update gated by the running state (order-aware, coarse-to-fine).
# Replaces the parallel weighted fuse; no outer alpha (--ngf_layer_weight_mode unused).
#
...
RESULT_DIR=./results_ov2_calvin_ngf_v2c_seq
EVAL_IN=./eval_results/ov2_calvin_ngf_v2c_seq_indomain
EVAL_X=./eval_results/ov2_calvin_ngf_v2c_seq_to_droid

echo "==== Arch C train: NGF sequential depth-recurrent refinement on CALVIN-1p ===="
